...
class YandexImagesDownloader():
    ...
    def download_images_by_page(self, keyword, page, imgs_count,
                                sub_directory,searchByImage) -> PageResult:
        ...
        self.check_captcha_and_get(YandexImagesDownloader.MAIN_URL,searchByImage,params=self.get_url_params(page, keyword))
    ...
    def download_images_by_keyword(self, keyword,searchByImage,sub_directory="") -> KeywordResult:
        ...
        self.check_captcha_and_get(YandexImagesDownloader.MAIN_URL,searchByImage,
                                   params={
                                       'text': keyword,
                                       "nomisspell": 1
                                   })
    ...
    def download_images(self, keywords: List[str],searchByImage) -> DownloaderResult:
        ...
        for keyword in keywords:
            ...
            keyword_result = self.download_images_by_keyword(
                keyword,searchByImage, sub_directory=keyword)
    ...
    def makeurl(self,keyword):

        defaultInputImagesPath="./putYourImagesHere/"

       

        if not os.path.exists(defaultInputImagesPath+keyword):
            raise Exception(f"the file named {keyword}doesnt exist")
            
        searchUrl = 'https://yandex.ru/images/search'
        files = {'upfile': ('blob', open(defaultInputImagesPath+keyword, 'rb'), 'image/jpeg')}
        params = {'rpt': 'imageview', 'format': 'json', 'request': '{"blocks":[{"block":"b-page_type_search-by-image__link"}]}'}
        response = requests.post(searchUrl, params=params, files=files)
        query_string = json.loads(response.content)['blocks'][0]['params']['url']
        img_search_url= searchUrl + '?' + query_string
        img_url_correction=img_search_url[:-9]+"imagelike"
        return img_url_correction

    def check_captcha_and_get(self, url, searchByImage,params=None):
        """Checking for captcha on url and get url after that.
        If there is captcha, you have to type it in input() or quit."""
        url_with_params=self.makeurl(params['text']) if searchByImage else f"{url}?{urlencode(params)}"
